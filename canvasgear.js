 ...
 * Version : v0.2.8 — 20240427°1523 — Formal updates
 ...
 * @note • See 🔪 for algorithm sections • see ✂ for the shared bottom part.
 */
...
Cvgr.Const =
{
    ...
    * @type {string} —
    */
   , versiontimestamp : '20240427°1523'
...
Cvgr.Vars =
{
   ...
   /**
    * This number stores the CanvasGear start milliseconds — Unused so far
    *
    * @id 20180618°0644
    * @type {number} (integer) —
    */
 ...
 * @param {number} iThick — Optional, width in pixel
 */
Cvgr.Objs.Line = function(iX1, iY1, iX2, iY2, sColor, iThick)
{
   ...
   if ( typeof iThick === 'undefined' ) {                              // [mark 20210517°1111`zz] Undefined check
       iThick = 2;
   }
...
Cvgr.startCanvasGear = function()
{
   ...
   // Workaround for missing requestAnimFrame [seq 20140815°0651]
 ...
 * @id 20190329°0931
 * @callers Only • executeFrame()
 * @param {number} iTimeCurr —
 * @param {number} iElapsedTwoSeconds —
 * @param {number} iFramesPerSecondTotal —
 */
...
Cvgr.Func.executeFrame = function()
{
   ...
   // Process each icon on the page [seq 20140815°1256]
   for (var iNdx = 0; iNdx < Cvgr.Vars.icos.length; iNdx++)
   {
      ...
      else
      {
         ...
         Cvgr.Vars.aPiggyModuleNamesTwo.push(sModuleNameTwo);          ////// e.g. non-existing "http://localhost/downtown.base/daftaridev/trunk/daftari/jslibs/canvasgear/canvasgear.triangle.js"
 ...
 *  (This is now built-in, but planned to be extracted as a rider script)
 *
 * @id 20180619°0121
 * @c_o_n_s_t — Namespace
 */
Cvgr.Algos.Ballist = {
    ...
    * @param {string} sRingName —
    * @param {number} nRadiusAbs —
    * @param {string} sColorRing —
    * @param {string} sColorSpace —
    * @returns {undefined} —
    */
    ...
    *
    * @id 20140916°0911
    * ////@_p_a_r_a_m {} nRadius — The target's radius in m, usually goes with lowest ring
    * @constructor —
    * @returns {undefinied} —
    */
   ...
   , executeAlgo_getSeries : function(sSeries)                         // Cvgr.Algos.Ballist.executeAlgo_getSeries
   {
      ...
      if (( typeof sSeries === 'undefined' ) || (sSeries.length < 1)) { // [mark 20210517°1111`zz] Undefined check
    ...
    * @param {Array} iko — This is Cvgr.Vars.icos[iNdx] at the caller.
    * ////@_param {number} iNdx — The index into the Cvgr.Vars.icos array.
    * @returns {undefined} —
    */
...
/** + + + 🔪 + + + + + + + + + + + + + + + + + + + + + + + + + +
 * This section/module provides the oblongrose algorithm
 *
 * id : 20190329°0721
 * version :
 * license : GNU LGPL v3 or later (https://www.gnu.org/licenses/lgpl.html)
...
/**
 * Formal integration of cut-out into main script
 *
 * @id 20190329°0621`xx
 * @type Object —
 * @c_o_n_s_t — Namespace
...
Cvgr.Algos.oblongrose = {
    ...
    * @param {number} iko — This is Cvgr.Vars.icos[iNdx] at the caller
    * @returns {undefined} —
    */
...
Cvgr.Algos.pulse = {
    ...
    * @status
    * @param {Array} iko — Array of icon objects, Cvgr.Vars.icos[iNdx] at the caller
    * @returns {undefined} —
    */
   executeAlgorithm : function(iko) // Cvgr.Algos.pulse.executeAlgorithm
   {
      ...
      // (.) Calculate progression [seq 20140829°0519]
      //  Note todo 20190329°0833 'centralize progression'
      iko.Angle += Cvgr.Vars.nIncTurnsPerFrame * Math.PI * iko.Hertz;
...
Cvgr.Algos.triangle = {
    ...
    * @param {Object} iko — This is Cvgr.Vars.icos[iNdx] at the caller
    * @return {undefined}
    */
   executeAlgorithm : function(iko) // Cvgr.Algos.triangle.executeAlgorithm
   {
      ...
      // pPreparatory calculations [seq 20140916°0823]
      ...
      // Try envelope against issue 20140901°0911 'Opera fillRect() fail' [seq 20140901°0912]
      try
      {
      ...
      // About why the try see issue 20140901°0933
      try
      {
...
Cvgr.Algos.triangulum = {
    ...
    * @param {Icon} iko — This is Cvgr.Vars.icos[iNdx] at the caller.
    * @returns {undefined} —
    */
 ...
 * @returns {string} — The wanted color hex value, e.g. '#FF0000' for 'red', or '#C0C0C0' silver for wrong names.
 */
