...
Cvgr.Const = {versionnumber:"0.2.0.c", versiontimestamp:"20190330\u00b01314", bShow_Debug_Dialogs:!1};
Cvgr.Func = {};
Cvgr.Objs = {};
...
Cvgr.Vars.aCallbackStockpile = [];
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(0);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(1);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(2);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(3);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(4);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(5);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(6);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(7);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(8);
});
Cvgr.Vars.aCallbackStockpile.push(function() {
  Cvgr.Func.executeFramContinue(9);
});
...
Cvgr.Func.examineAlgo = function(a, b) {
  "undefined" !== typeof a && (b.AlgoName in Cvgr.Algos ? Cvgr.Vars.aPiggySuccessFlags[a] = !0 : b.AlgoName = "pulse");
};
Cvgr.Func.executeFramContinue = function(a) {
  var b = Cvgr.Vars.aPiggyAlgoNames[a];
  Cvgr.Vars.sDebugPageHelper += "<br /> \u2014 executeFramConti : piggy no " + a;
  a = Cvgr.Vars.aPiggyIconArrays[a];
  Cvgr.Vars.sDebugPageHelper += ", length " + a.length + ', algo "' + a[0].AlgoName + '"';
  for (var c = 0; c < a.length; c++) {
    var d = a[c];
    Cvgr.Vars.sDebugPageHelper += "<br /> \u2014 &nbsp; &nbsp; &nbsp; &nbsp; iko.Ide = " + d.Ide;
    b in Cvgr.Algos && (d.bIsDefaultSettingDone || Cvgr.Func.settleAlgoProperties(d), Cvgr.Algos[d.AlgoName].executeAlgorithm(d));
  }
};
  ...
  for (a = 0; a < Cvgr.Vars.icos.length; a++) {
    if (c = Cvgr.Vars.icos[a], Cvgr.Func.executeFram_PrintInfoCanvas(a), b = c.AlgoName, b in Cvgr.Algos && "Template" !== b || c.bIsDefaultSettingDone) {
      ...
      if (0 <= d) {
        ...
        var e = Cvgr.Vars.aPiggyAlgoNames.length;
        Cvgr.Vars.aPiggyAlgoNames.push(b);
        var g = [];
        g.push(c);
        Cvgr.Vars.aPiggyIconArrays.push(g);
        Cvgr.Vars.aPiggySuccessFlags.push(!1);
        Cvgr.Vars.aPiggyTimers.push(setTimeout(Cvgr.Func.examineAlgo, 1357, Cvgr.Vars.aPiggyTimers.length - 1, c));
        Cvgr.Vars.sDebugPageHelper += "<br /> \u2014 pullScriptBehind " + e + " " + b;
        Trekta.Utils.pullScriptBehind(d, Cvgr.Vars.aCallbackStockpile[e]);
      }
    }
  }
...
Cvgr.Func.settleAlgoProperties = function(a) {
  var b = {algo:"AlgoName", bgcolor:"BgColor", color:"Color", color2:"Color2", color3:"Color3", height:"Height", hertz:"Hertz", width:"Width"};
  try {
