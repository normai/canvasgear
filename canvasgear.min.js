...
Cvgr.Objs.Ikon = function() {
  ...
  this.Width = this.Height = this.Speed = this.SizeFactor = this.ShiftY = this.ShiftX = this.Ide = this.Hertz = null;
  this.Angle = 0;
  this.CmdHash2 = this.Canvas = null;
  this.Command = "";
  this.Context = null;
  this.DrawOnlyOnce = !1;
  this.iDrawCount = 0;
...
Cvgr.Func.executeFramContinue = function(a, b) {
  a in Cvgr.Algos && Cvgr.Algos[a].executeAlgorithm(Cvgr.Vars.icos[b]);
};
Cvgr.Func.executeFram_PrintInfo = function(a) {
  var b = document.getElementById(Cvgr.Vars.icos[a].Ide + ".info");
  if (null !== b) {
    var c = "<small>Canvas Debug Info :" + ("<br />iko.Angle = " + Cvgr.Vars.icos[a].Angle.toFixed(9) + " <br />iko.Color = " + Cvgr.Vars.icos[a].Color + "<br />iko.Height = " + Cvgr.Vars.icos[a].Height + "<br />iko.Mode = " + (Cvgr.Vars.bFlagTipTopTest ? "Top" : "Tip") + "<br />iko.Width = " + Cvgr.Vars.icos[a].Width);
    for (var d in Cvgr.Vars.icos[a].CmdHash2) {
      var e = Trekta.Utils.htmlEscape(Cvgr.Vars.icos[a].CmdHash2[d]);
      c += "<br /> [cmd] " + d + " = " + e;
    }
    b.innerHTML = c + "</small>";
  }
};
Cvgr.Func.executeFram_PrintPageInfo = function(a, b, c) {
...
Cvgr.Func.executeFrame = function() {
  ...
  Cvgr.Func.executeFram_PrintPageInfo(a, b, c);
  for (var d = 0; d < Cvgr.Vars.icos.length; d++) {
    Cvgr.Func.executeFram_PrintInfo(d), a = Cvgr.Vars.icos[d].AlgoName, a in Cvgr.Algos ? Cvgr.Algos[a].executeAlgorithm(Cvgr.Vars.icos[d]) : (a = Trekta.Utils.retrieveScriptFolderAbs("canvasgear.js") + "canvasgear." + a + ".js", Cvgr.Vars.timSuccess.push(!1), Cvgr.Vars.timrs.push(setTimeout(Cvgr.Func.examineAlgo, 1357, Cvgr.Vars.timrs.length - 1, d)), Trekta.Utils.pullScriptBehind(a, function() {
      Cvgr.Func.executeFramContinue(d);
    }));
  }
...
Cvgr.Algos.Ballist = {Ring:function(a, b, c, d) {
  ...
  if (!a.DrawOnlyOnce) {
    a.DrawOnlyOnce = !0;
    var b = Cvgr.Algos.Ballist.executeAlgo_getTarget(), c = Cvgr.Algos.Ballist.executeAlgo_getSeries(a.CmdHash2.series);
...
Cvgr.Algos.develop = {executeAlgorithm:function(a) {
  if (!a.DrawOnlyOnce) {
    ...
    a.Context.fillRect(0, 0, a.Canvas.width, a.Canvas.height);
    var c = [], d = new Cvgr.Objs.Line(3, 3, b - 3, 3, "crimson"), e = new Cvgr.Objs.Line(4, b - 4, b - 4, b - 4, "seagreen");
    b = new Cvgr.Objs.Line(5, b - 7, b - 5, 7, "royalblue");
    c.push(d);
    c.push(e);
    c.push(b);
...
Cvgr.Algos.pulse = {executeAlgorithm:function(a) {
  ...
  b *= a.SizeFactor;
  b /= 2;
  var c = b * Math.abs(Math.cos(a.Angle));
  var d = b + parseInt(a.ShiftX, 10);
  b += parseInt(a.ShiftY, 10);
  a.Context.beginPath();
  a.Context.arc(d, b, c, 0, 2 * Math.PI, !1);
  a.Context.closePath();
  a.Context.fillStyle = a.Color;
  a.Context.fill();
  a.Angle += Cvgr.Vars.nIncTurnsPerFrame * Math.PI * a.Hertz;
