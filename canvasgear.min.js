...
Cvgr.Const = {versionnumber:"0.2.0.b", versiontimestamp:"20190330\u00b00757..", bShow_Debug_Dialogs:!1};
Cvgr.Func = {};
Cvgr.Objs = {};
Cvgr.Vars = {bFlagTipTopTest:!1, bIsFirstFrame:!0, iTimeStart:0, iTimeStartMs:0};
...
Cvgr.Vars.aDebugCallback = [];
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(0, "a");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(1, "b");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(2, "c");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(3, "d");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(4, "e");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(5, "f");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(6, "g");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(7, "h");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(8, "i");
});
Cvgr.Vars.aDebugCallback.push(function() {
  Cvgr.Func.executeFramContinue(9, "j");
});
Cvgr.Vars.aDebugCbAlgoNam = "         ".split(" ");
...
Cvgr.Func.examineAlgo = function(a, b) {
  "undefined" !== typeof a && (b.AlgoName in Cvgr.Algos ? Cvgr.Vars.aPullSuccess[a] = !0 : b.AlgoName = "pulse");
};
Cvgr.Func.executeFramContinue = function(a, b) {
  var c = Cvgr.Vars.aPullAlgos[a];
  Cvgr.Vars.sDebugPageHelper += "<br /> \u2014 executeFramConti " + a + " " + b;
  b = Cvgr.Vars.aPullPiggy[a];
  Cvgr.Vars.sDebugPageHelper += "<br /> \u2014 &nbsp; &nbsp; &nbsp; &nbsp; piggy.length = " + b.length + " " + Cvgr.Vars.aDebugCbAlgoNam[a];
  for (a = 0; a < b.length; a++) {
    var d = b[a];
    c in Cvgr.Algos && (d.bIsDefaultSettingDone || Cvgr.Func.settleAlgoProperties(d), Cvgr.Algos[d.AlgoName].executeAlgorithm(d));
  }
  Cvgr.Vars.bIsFirstFrame = !1;
};
...
Cvgr.Func.executeFrame = function() {
  ...
  for (a = 0; a < Cvgr.Vars.icos.length; a++) {
    if (c = Cvgr.Vars.icos[a], Cvgr.Func.executeFram_PrintInfoCanvas(a), b = c.AlgoName, b in Cvgr.Algos && "Template" !== b || c.bIsDefaultSettingDone) {
      c.bIsDefaultSettingDone || Cvgr.Func.settleAlgoProperties(c);
      try {
        Cvgr.Algos[b].executeAlgorithm(c);
      } catch (f) {
      }
    } else {
      var d = Cvgr.Vars.aPullAlgos.indexOf(b);
      if (0 <= d) {
        2 > Cvgr.Vars.iFrameNo && Cvgr.Vars.aPullPiggy[d].push(c);
      } else {
        d = Trekta.Utils.retrieveScriptFolderAbs("canvasgear.js") + "canvasgear." + b + ".js";
        var e = Cvgr.Vars.aPullAlgos.length;
        Cvgr.Vars.aPullAlgos.push(b);
        var g = [];
        g.push(c);
        Cvgr.Vars.aPullPiggy.push(g);
        Cvgr.Vars.aPullSuccess.push(!1);
        Cvgr.Vars.aPullTimrs.push(setTimeout(Cvgr.Func.examineAlgo, 1357, Cvgr.Vars.aPullTimrs.length - 1, c));
        Cvgr.Vars.sDebugPageHelper += "<br /> \u2014 pullScriptBehind " + e + " " + b;
        Cvgr.Vars.aDebugCbAlgoNam[e] = b;
        Trekta.Utils.pullScriptBehind(d, Cvgr.Vars.aDebugCallback[e]);
      }
    }
  }
  window.requestAnimFrame(Cvgr.Func.executeFrame);
};
Cvgr.Func.settleAlgoProperties = function(a) {
  var b = {algo:"AlgoName", bgcolor:"BgColor", height:"Height", hertz:"Hertz", width:"Width"};
  try {
    var c = Cvgr.Algos[a.AlgoName].defaultProperties;
  } catch (g) {
    return;
  }
  for (var d in c) {
  ...
  for (var e in a.CmdsHash) {
    c = e, e in b && (c = b[e]), a[c] = a.CmdsHash[e];
  }
