...
Cvgr.Func.executeFramContinue = function(a) {
  var b = "";
  try {
    b = a.AlgoName;
  } catch (c) {
  ...
  b in Cvgr.Algos && (Cvgr.Vars.bIsNotFirstFrame || Cvgr.Func.settleAlgoProperties(a), Cvgr.Algos[a.AlgoName].executeAlgorithm(a));
...
Cvgr.Func.executeFrame = function() {
  ...
  for (a = 0; a < Cvgr.Vars.icos.length; a++) {
    var d = Cvgr.Vars.icos[a];
    Cvgr.Func.executeFram_PrintInfo(a);
    c = d.AlgoName;
    c in Cvgr.Algos && "Template" !== c || Cvgr.Vars.bIsNotFirstFrame && "Template" === c ? (Cvgr.Vars.bIsNotFirstFrame || Cvgr.Func.settleAlgoProperties(d), Cvgr.Algos[c].executeAlgorithm(d)) : (b = Cvgr.Vars.aPullAlgos.indexOf(c), 0 <= b ? Cvgr.Vars.aPullPiggy[b].push(d) : (b = Trekta.Utils.retrieveScriptFolderAbs("canvasgear.js") + "canvasgear." + c + ".js", Cvgr.Vars.aPullAlgos.push(c), c = [], c.push(d), Cvgr.Vars.aPullPiggy.push(c), Cvgr.Vars.aPullSuccess.push(!1), Cvgr.Vars.aPullTimrs.push(setTimeout(Cvgr.Func.examineAlgo, 
    1357, Cvgr.Vars.aPullTimrs.length - 1, d)), Trekta.Utils.pullScriptBehind(b, function() {
...
Cvgr.Func.settleAlgoProperties = function(a) {
  var b = {algo:"AlgoName", height:"Height", hertz:"Hertz", width:"Width"}, c = Cvgr.Algos[a.AlgoName].defaultProperties, d;
  for (d in c) {
    a[d] = c[d];
  }
