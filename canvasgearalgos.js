...
/**
 * This namespace shall hold the ballistics algorithm
 *
 * @id 20180619°0121
 */
 ...
 * @param sColorSpace {}
 */
Cvgr.Algs.Bal.Ring = function(sRingName, nRadiusAbs, sColorRing, sColorSpace ) {
 ...
 * @param nMinutes {}
 */
Cvgr.Algs.Bal.Hit = function(nRingval, nMinutes) {
...
/**
 * This function implements a drawing algorithm
 *
 * @id 20140916°0421
 * @status under construction
 * @note The statement 'fill() includes closePath()' is true only to some
 *         degree, e.g. *not* for drawing the final line to origin.
 * @note Compatibility: This seems not yet to work with IE8.
 * @callers • Cvgr.Func.executeFrame
 * @param {array} icos — This is Cvgr.Vars.icos[iFor] at the caller.
 * @param {number} iFor — The index into the Cvgr.Vars.icos array.
 */
Cvgr.Algs.Bal.algoBallist = function(icos, iFor)
{
   // prolog
   ...
   // retrieve target
   var tgt = Cvgr.Algs.Bal.algoBallist_getTarget();

   // retrieve series
   var hits = Cvgr.Algs.Bal.algoBallist_getSeries(iko.CmdHash2['series']);
   ...
   for (var iLoop = 0; iLoop < tgt.rings.length; iLoop++) {
      ...
      var radius = iCenterPoint * tgt.rings[iLoop].radiusAbs * 12; // REPLACE FIXED FACTOR BY CALCULATED FACTOR
   ...
   Cvgr.Algs.Bal.algoBallist_drawDiagonal(iko);
 ...
 * @param {object} iko ...
 */
Cvgr.Algs.Bal.algoBallist_drawDiagonal = function(iko)
{
   ...
   var nSize = (nHeight + nWidth) / 2;         // calculated flat rate
    ...
    *
    * @id 20140926°1212
    * @note Not sure, what exactly this may be good for. Just eliminate it?
    */
   var nSiz = 1;
 ...
 * @id 20140926°0911
 * @status
 * @callers Only Cvgr.Algs.Bal.algoBallist
 * @param sSeries {} ..
 */
Cvgr.Algs.Bal.algoBallist_getSeries = function(sSeries)
{
   var hits = new Array();
   ...
   if (sSeries.length < 1) {
      ...
      var h = new Cvgr.Algs.Bal.Hit(10.7, 55); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(9.3, 43); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.1, 0); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.2, 1); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.3, 3); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.4, 6); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.5, 10); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.6, 20); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.7, 30); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.8, 40); hits.push(h);
      var h = new Cvgr.Algs.Bal.Hit(2.9, 50); hits.push(h);
      ...
      for (var i = 0; i < a1.length; i++) {
         var a2 = a1[i].split("/");
         var h = new Cvgr.Algs.Bal.Hit(a2[0], a2[1]); hits.push(h);
      }
   }
 ...
 * @id 20140916°0921
 * @status Under construction
 * @note function Cvgr.Algs.Bal.Ring(sRingName, nRadiusAbs, sColorRing, sColorSpace )
 * @note The details are still be to adjusted.
 * @param sTargetName {string} ..
 */
 Cvgr.Algs.Bal.algoBallist_getTarget = function(sTargetName)
{
   var target = new Cvgr.Algs.Bal.Target();
   ...
   if (sTargetName === 'kkspp') {
      ...
      target.rings.push(new Cvgr.Algs.Bal.Ring('10', 0.025, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '9', 0.050, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '8', 0.075, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '7', 0.100, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '6', 0.125, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '5', 0.150, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '4', 0.175, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '3', 0.200, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '2', 0.225, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '1', 0.250, 'blue' , 'white'));
   }
   else if (sTargetName === 'kkspd') {
      target.Diameter = 0.500;
      target.Naame = "Sportpistole 25 m Duell";
      target.Shortnam = sTargetName;
      target.rings.push(new Cvgr.Algs.Bal.Ring( '5', 0.050, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '4', 0.100, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '3', 0.150, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '2', 0.200, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '1', 0.250, 'blue' , 'black'));
   }
   else if (sTargetName === 'lg10m') {
      target.Diameter = 0.050;
      target.Naame = "Luftgewehr 10 m";
      target.Shortnam = sTargetName;
      target.rings.push(new Cvgr.Algs.Bal.Ring('10', 0.0025, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '9', 0.0050, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '8', 0.0075, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '7', 0.0100, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '6', 0.0125, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '5', 0.0150, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '4', 0.0175, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '3', 0.0200, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '2', 0.0225, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '1', 0.0250, 'blue' , 'white'));
   }
   else if (sTargetName === 'lgdt10m') {
      target.Diameter = 0.0100;
      target.Naame = "Deutsche Luftgewehr-Scheibe 10 m";
      target.Shortnam = sTargetName;
      target.rings.push(new Cvgr.Algs.Bal.Ring('10', 0.0005, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '9', 0.0010, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '8', 0.0015, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '7', 0.0020, 'blue' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '6', 0.0025, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '5', 0.0030, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '4', 0.0035, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '3', 0.0040, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '2', 0.0045, 'blue' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '1', 0.0050, 'blue' , 'white'));
   }
   else {
      target.Diameter = 0.1500;
      target.Naame = "Luftpistole 10 m";
      target.Shortnam = 'lupi10m';
      target.rings.push(new Cvgr.Algs.Bal.Ring('10', 0.0055, 'black', 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '9', 0.0135, 'black', 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '8', 0.0215, 'gray' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '7', 0.0295, 'gray' , 'black'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '6', 0.0375, 'gray' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '5', 0.0455, 'gray' , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '4', 0.0535, 'red'  , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '3', 0.0615, 'red'  , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '2', 0.0695, 'red'  , 'white'));
      target.rings.push(new Cvgr.Algs.Bal.Ring( '1', 0.0775, 'red'  , 'white'));
   }
